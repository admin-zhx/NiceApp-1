...
/**
 * User: shine Date: 2014-12-12 Time: 16:48 Description:
 */
public class RhythmLayout extends HorizontalScrollView {
	...
	private void updateItemHeight(float scrollX) {
		...
		if (position == mCurrentItemPosition
				|| position >= mLinearLayout.getChildCount())
			return;
	...
	private void makeItems(int fingerPosition, List<View> viewList) {
		...
		for (int i = 0; i < size; i++) {
			...
			int translationY = Math.min(Math.max(Math.abs(fingerPosition - i)
					* mIntervalHeight, 10), mMaxTranslationHeight);
			// 位移动画
	 ...
	 * 
	 * @param isForward
	 *            是否获取firstPosition - 1 位置的小图标
	 * @param isBackward
	 *            是否获取lastPosition + 1 位置的小图标
	 * @return
	 */
	...
	class ShiftMonitorTimer extends Timer {
		...
		void monitorTouchPosition(float x, float y) {
			...
			if ((x < 0.0F)
					|| ((x > mEdgeSizeForShiftRhythm) && (x < mScreenWidth
							- mEdgeSizeForShiftRhythm)) || (y < 0.0F)) {
				mFingerDownTime = System.currentTimeMillis();
				this.canShift = false;
			} else {
				this.canShift = true;
					...
					public void run() {
						...
						if (canShift && duration > 1000) {
							...
							if (isForward || isBackward) {
								localList = getVisibleViews(isForward,
										isBackward);
								final int finalToPosition = toPosition;
								mHandler.post(new Runnable() {
									@Override
									public void run() {
										makeItems(mCurrentItemPosition,
												localList);// 设置每个Item的高度
										scrollToPosition(finalToPosition, 200,
												0, true);// 设置ScrollView在x轴的坐标
										vibrate(10L);
									}
								});
							}
	 ...
	 * @return
	 */
	public Animator scrollToPosition(int position, int duration,
			int startDelay, boolean isStart) {
	 ...
	 * @return
	 */
	public Animator scrollToPosition(int position, int startDelay,
			boolean isStart) {
		int viewX = (int) getItemView(position).getX();
		return smoothScrollX(viewX, 300, startDelay, isStart);
	}

	private Animator smoothScrollX(int position, int duration, int startDelay,
			boolean isStart) {
		return AnimatorUtils.moveScrollViewToX(this, position, duration,
				startDelay, isStart);
	}
	...
	private void updateItemHeightAnimator(View view, int translationY) {
		if (view != null)
			AnimatorUtils.showUpAndDownBounce(view, translationY, 180, true,
					true);
	}
	...
	public Animator shootDownItem(int viewPosition, boolean isStart) {
		if ((viewPosition >= 0) && (mLinearLayout != null)
				&& (getSize() > viewPosition))
	...
	public Animator shootDownItem(View view, boolean isStart) {
		if (view != null)
			return AnimatorUtils.showUpAndDownBounce(view,
					mMaxTranslationHeight, 350, isStart, true);
		return null;
	}
	...
	public Animator bounceUpItem(View view, boolean isStart) {
		if (view != null)
			return AnimatorUtils.showUpAndDownBounce(view, 10, 350, isStart,
					true);
		return null;
	}
	...
	public void showRhythmAtPosition(int position) {
		...
		if ((this.mLastDisplayItemPosition < 0) || (mAdapter.getCount() <= 7)
				|| (position <= 3)) {
			...
			// 当前要位移到的位置为最后3个
			scrollAnimator = scrollToPosition(mAdapter.getCount() - 7,
					mScrollStartDelayTime, false);
		} else {
			// 当前位移到的位置既不是前3个也不是后3个
			scrollAnimator = scrollToPosition(position - 3,
					mScrollStartDelayTime, false);
		}
		// 获取对应item升起动画
		...
		animatorSet2.playSequentially(new Animator[] { scrollAnimator,
				animatorSet1 });
		animatorSet2.start();
