...
public class CardFragment extends AbsBaseFragment {
	...
	protected View initViews(LayoutInflater paramLayoutInflater) {
		...
		mBottomEdgeImageView = ((ImageView) view
				.findViewById(R.id.image_bottom_edge));
		...
		this.mAuthorText.setText(Html.fromHtml("<B>"
				+ this.mCard.getAuthorName() + "</B>"));
	...
	protected void initAndDisplayCoverImage() {
		int coverWidth = AppUtils.getScreenDisplayMetrics(getActivity()).widthPixels
				- 2 * getResources().getDimensionPixelSize(R.dimen.card_margin);
		int coverHeight = (int) (180.0F * (coverWidth / 320.0F));
		ViewGroup.LayoutParams localLayoutParams = this.mCoverImageView
				.getLayoutParams();
		localLayoutParams.height = Float.valueOf(coverHeight).intValue();
		// 加载图片
		int picResource = AppUtils.getDrawableIdByName(getActivity(),
				mCard.getCoverImgerUrl());
