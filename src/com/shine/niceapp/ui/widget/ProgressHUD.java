...
public class ProgressHUD extends Dialog {
	...
	public static ProgressHUD show(Context context,
			CharSequence textCharSequence, int drawableRes,
		...
		if ((textCharSequence == null) || (textCharSequence.length() == 0))
			progressHUD.findViewById(R.id.text_message)
					.setVisibility(View.GONE);
		else {
			progressHUD.imageView = ((ImageView) progressHUD
			...
			WindowManager.LayoutParams localLayoutParams = progressHUD
					.getWindow().getAttributes();
			localLayoutParams.dimAmount = 0.2F;
			progressHUD.getWindow().setAttributes(localLayoutParams);
			((TextView) progressHUD.findViewById(R.id.text_message))
					.setText(textCharSequence);
			progressHUD.show();
		}
		return progressHUD;
	}
