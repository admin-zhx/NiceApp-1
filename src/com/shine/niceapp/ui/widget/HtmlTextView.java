...
public class HtmlTextView extends TextView {
	...
	protected void onLayout(boolean paramBoolean, int paramInt1, int paramInt2,
		...
		if ((paramBoolean) || (this.mNeedsResize))
			resizeText(paramInt3 - paramInt1 - getCompoundPaddingLeft()
					- getCompoundPaddingRight(), paramInt4 - paramInt2
	...
	public void resizeText(int paramInt1, int paramInt2) {
		...
		if ((localCharSequence == null) || (localCharSequence.length() == 0)
				|| (paramInt2 <= 0) || (paramInt1 <= 0))
			return;
		TextPaint localTextPaint = getPaint();
		StaticLayout localStaticLayout = new StaticLayout(localCharSequence,
				localTextPaint, paramInt1, Layout.Alignment.ALIGN_NORMAL,
				1.25F, 0.0F, true);
		...
		if (i > j) {
			...
			while (paramInt1 < f1 + f2) {
				m--;
				f1 = localTextPaint.measureText(localCharSequence.subSequence(
						k, m + 1).toString());
			}
